...
async def update_user_preferences(
    ...
    try:
        ...
        # Persist feed_tags if provided by the Android app
        if prefs.feed_tags is not None:
            update_payload["feed_tags"] = prefs.feed_tags

        user_ref.set(update_payload, merge=True)

        # 2. Trigger the instant scraper in the background, passing tag context
