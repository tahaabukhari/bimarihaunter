...
$fullImage = "gcr.io/$PROJECT_ID/bimari_haunter:$IMAGE_TAG"

Write-Host "Building and submitting Cloud Build..."
& gcloud builds submit --tag $fullImage --project $PROJECT_ID

Write-Host "Deploying to Cloud Run as service 'bimari_haunter'..."
& gcloud run deploy bimari_haunter --image $fullImage --region $REGION --platform managed --allow-unauthenticated --project $PROJECT_ID
